...
module RubyNeuralNets
  ...
  module DataLoaders
    ...
    # Dataset of classified images
    class ClassifiedImages < DataLoader
      ...
      #
      # Parameters::
      # * *name* (String): Name of the dataset to read (sub-directory of the dataset directory)
      ...
      def initialize(name, percent_training: 0.7, percent_dev: 0.15)
        @name = name
        @types = %i[train dev test]
        @classes = Dir.glob("#{root}/*").
          select { |file| File.directory?(file) }.
          map { |file| File.basename(file) }.
          sort
        raise "No data in dataset #{name}" if @classes.empty?
        randomized_dataset = @classes.map do |class_name|
          Dir.glob("#{root}/#{class_name}/*.png").map do |file|
            [
              file,
              class_name
            ]
      ...
      # List the dataset files for a given type and class
      #
      # Parameters::
      # * *dataset_type* (Symbol): Dataset type
      # * *class_name* (String): Class name
      # result::
      # * Array< String >: List of file paths
      def files(dataset_type, class_name)
        files = []
        @datasets[dataset_type].each do |select_file, select_class_name|
          files << select_file if select_class_name == class_name
        end
        files
      end
      ...
      def for_each_minibatch(dataset_type, max_minibatch_size)
        ...
        while !remaining_dataset.empty?
          ...
          yield(
            @minibatch_cache[idx_minibatch][:x],
            @minibatch_cache[idx_minibatch][:y],
            @minibatch_cache[idx_minibatch][:x].shape[1]
          )
          idx_minibatch += 1
        end
      end
